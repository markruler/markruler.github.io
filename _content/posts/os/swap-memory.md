...
이 글은 **Ubuntu**에서 **스왑 메모리를 file로 설정**하고, 나의 환경에 맞게 조정하기 위해 **모니터링 하는 방법**을 설명한다.
...
*Ubuntu 24.04 기준*으로는 기본적으로 다음과 같이 `/swap.img`가 설정되어 있다.
하지만 이 글은 설정되어 있지 않다는 전제로 진행한다.
