...
- [image라는 단어](#image라는-단어)
  - [화질 (Image Quality)](#화질-image-quality)
...
> IT에서는 시스템, 파일, 환경의 상태를 하나의 단일 단위로 묶은 복제본을
...
> IT에서는 특정 시점의 상태를 저장한 것을 말합니다.

## 화질 (Image Quality)

화질이란 이미지의 시각적 품질을 의미합니다.
화질을 결정하는 요소들은 일상 생활에서도 익숙하게 볼 수 있습니다.

**해상도(Resolution)** 는 디지털 이미지의 픽셀 단위 크기를 의미합니다.
1920×1080처럼 가로와 세로 방향의 픽셀 수로 표현되어
...
PNG(Portable Network Graphics)는 **무손실 압축**을 지원하는 래스터 이미지 포맷입니다.
PKZIP(`.zip`)처럼 Deflate 알고리즘(LZ77+허프만 코딩)으로 압축하여 품질 저하 없이 용량을 줄입니다.
트루 컬러(24비트)와 인덱스 컬러(8비트) 모드를 지원하며, **투명도를 표현**할 수 있습니다.
특허가 있는 LZW 압축 알고리즘을 사용하는 GIF를 대체하기 위해 개발되었다고 합니다.
...
픽셀 대신 **벡터(vector) 형태로 그래픽을 표현**하는 이미지 포맷입니다.
...
결론적으로
**사진**에는 JPEG 또는 WebP/AVIF,
**로고나 아이콘**에는 SVG 또는 PNG,
**애니메이션**에는 GIF 또는 WebP를 사용합니다.
...
이를 줄이기 위해 적절한 사이즈의 이미지 스프라이트를 사용할 수 있습니다.

## 브라우저 캐싱

`Cache-Control` 헤더를 설정하여 한 번 받은 이미지를 사용자가 재방문 시 재활용하도록 합니다.
변경이 드문 자원은 `max-age`를 길게 주고, 파일명에 버전 해시를 붙여 캐시 갱신을 관리할 수 있습니다.
...
필요 시 해당 영역을 아예 불러오지 않도록 처리하기도 쉽습니다.
예를 들면 미디어 쿼리를 사용해서 모바일 환경에서는 요소를 불러오지 않도록 할 수 있습니다.
또한 `<picture>` 요소를 사용하면 `<source media>`로 다양한 상황별 이미지를 지정할 수도 있습니다.
...
첫째, **시각 장애가 있는 사용자들**은 웹페이지를 스크린 리더(screen reader)를 통해 탐색합니다.
...
**이미지의 내용과 맥락을 정확하게 설명**해야 합니다.
예를 들어 제품 이미지라면 단순히 "스마트폰"이 아니라 "파란색 케이스를 씌운 스마트폰 정면"처럼 구체적인 묘사가 필요합니다.

"사진", "그림"과 같은 표현은 **굳이 포함하지 않아도 됩니다**.
스크린 리더는 해당 요소가 이미지라는 것을 이미 인식하고 있으므로 중복된 설명은 피하는 것이 좋습니다.

이미지가 **링크 역할을 할 경우**, 단순한 설명보다는 **링크의 목적지나 기능**을 설명해야 합니다.
예를 들어 "장바구니"보다 "장바구니 페이지로 이동"이라고 작성합니다.

반대로, 이미지가 **순수하게 장식용**이라면
`alt` 속성을 빈 값(`alt=""`)으로 설정해야 스크린 리더가 그것을 무시할 수 있습니다.
시각적으로만 의미 있는 요소까지 읽게 되면 오히려 사용자에게 혼란을 줄 수 있기 때문입니다.
