...
제가 다룰 예시에서는 (수많은) 설정 옵션 없이 명령어의 기본 동작에 대해 설명하겠습니다. 😄
...
Git은 `9e78i`와 `035cc` 커밋에 반영된 변경 사항을 버리고
...
`dev` 브랜치의 `76d12` 커밋이 `master` 브랜치에서 원하는 변경 사항을
...
Awesome! 이제 원격 브랜치와 완전히 동기화되었고 최신 변경 사항이 모두 반영되었습니다! 🤩
